...
namespace Microsoft.Wim
{
	...
	public static partial class WimgApi
	{
		...
		public static WimHandle CommitImageHandle(WimHandle imageHandle, bool append, WimCommitImageOptions options)
		{
			...
			//
			if (!WimgApi.NativeMethods.WIMCommitImageHandle(imageHandle, append ? WimgApi.WIM_COMMIT_FLAG_APPEND : 0 | (DWORD)options, out newImageHandle))
			{
		...
		public static void CopyFile(string sourceFile, string destinationFile, WimCopyFileOptions options, CopyFileProgressCallback copyFileProgressCallback, object userData)
		{
			...
			//
			var fileInfoCopyProgress = new CopyFileProgress(sourceFile, destinationFile, copyFileProgressCallback, userData);
		...
		public static WimHandle CreateFile(string path, WimFileAccess desiredAccess, WimCreationDisposition creationDisposition, WimCreateFileOptions options, WimCompressionType compressionType)
		{
			...
			// Call the native function
			//
			WimHandle wimHandle = WimgApi.NativeMethods.WIMCreateFile(path, (DWORD)desiredAccess, (DWORD)creationDisposition, (DWORD)options, (DWORD)compressionType, out creationResult);
		...
		public static WimInfo GetAttributes(WimHandle wimHandle)
		{
			...
			//
			var wimInfoSize = (DWORD)Marshal.SizeOf(typeof(WimgApi.WIM_INFO));
		...
		public static IXPathNavigable GetImageInformation(WimHandle wimHandle)
		{
			...
			try
			{
				...
				if (str != null)
				{
					using (var stringReader = new StringReader(str.Substring(1)))
					{
		...
		public static WimHandle GetMountedImageHandle(string mountPath, bool readOnly, out WimHandle imageHandle)
		{
			...
			// Call the native function
			//
			if (!WimgApi.NativeMethods.WIMGetMountedImageHandle(mountPath, readOnly ? WimgApi.WIM_FLAG_MOUNT_READONLY : 0, out wimHandle, out imageHandle))
			{
				// Throw a Win32Exception based on the last error code
				//
				throw new Win32Exception();
		...
		public static WimMountInfoCollection GetMountedImageInfo()
		{
			...
			//
			WimgApi.NativeMethods.WIMGetMountedImageInfo(WimMountInfo.MountInfoLevel, out imageCount, IntPtr.Zero, 0, out returnLength);
		...
		public static WimMountInfo GetMountedImageInfoFromHandle(WimHandle imageHandle)
		{
			...
			try
			{
				...
				//
				if (!WimgApi.NativeMethods.WIMGetMountedImageInfoFromHandle(imageHandle, WimMountInfo.MountInfoLevel, mountInfoPtr, (DWORD)mountInfoSize, out returnLength))
				{
		...
		public static void SetImageInformation(WimHandle wimHandle, IXPathNavigable imageInfoXml)
		{
			...
			//
			string imageInfo = String.Format("\uFEFF{0}", imageInfoXml.CreateNavigator().OuterXml);
		...
		public static void SplitFile(WimHandle wimHandle, string partPath, long partSize)
		{
			...
			{
				throw new DirectoryNotFoundException(String.Format("Could not find part of the path '{0}'", Path.GetDirectoryName(partPath)));
			}
			...
			lock (WimgApi.LockObject)
			{
				...
				if (wimHandle != WimHandle.Null && messageCallback != null)
				{
					...
					if (!WimgApi.RegisteredCallbacks.IsCallbackRegistered(wimHandle, messageCallback))
					{
						// Throw an ArgumentOutOfRangeException
						//
						throw new ArgumentOutOfRangeException("messageCallback", "Message callback is not registered under this handle.");
					}
				}
